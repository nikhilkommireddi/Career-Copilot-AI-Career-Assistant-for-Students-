...
if st.button("Analyze") and resume_text and jd_text:
    ...
    st.markdown("**Skills in JD:** " + (", ".join(sorted(jd_sk)) or "None"))
    st.markdown("**Skills in Resume:** " + (", ".join(sorted(res_sk)) or "None"))
    st.markdown("**Missing Skills:** " + (", ".join(sorted(missing)) or "None"))
